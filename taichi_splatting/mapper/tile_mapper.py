...
def tile_mapper(config: RasterConfig, use_depth16=False):
    ...
    if use_depth16 is False:
        ...
        def make_sort_key(depth: ti.f32, tile_id: ti.i32):
            assert depth >= 0, f"depth {
                depth} cannot be negative for int 32 key!"
        ...
        for idx in range(cumulative_overlap_counts.shape[0]):
            ...
            for tile_uv in ti.grouped(ti.ndrange(*query.tile_span)):
                if query.test_tile(tile_uv):
                    ...
                    # map overlap index back to point index
                    overlap_to_point[key_idx] = idx
                    key_idx += 1
    ...
    def sort_tile_depths(depths: torch.Tensor, tile_overlap_ranges: torch.Tensor, cum_overlap_counts: torch.Tensor, total_overlap: int, image_size):
        ...
        overlap_key = torch.empty(
            (total_overlap, ), dtype=key_type, device=cum_overlap_counts.device)
        overlap_to_point = torch.empty(
            (total_overlap, ), dtype=torch.int32, device=cum_overlap_counts.device)
        ...
        overlap_key, overlap_to_point = cuda_lib.radix_sort_pairs(
            overlap_key, overlap_to_point, end_bit=end_sort_bit)
        return overlap_key, overlap_to_point

    def generate_tile_overlaps(gaussians, image_size):
        overlap_counts = torch.empty(
            (gaussians.shape[0], ), dtype=torch.int32, device=gaussians.device)

        if gaussians.shape[0] > 0:
            tile_overlaps_kernel(gaussians, ivec2(image_size), overlap_counts)

            cum_overlap_counts, total_overlap = cuda_lib.full_cumsum(
                overlap_counts)
    ...
    def f(gaussians: torch.Tensor, depths: torch.Tensor, image_size: Tuple[Integral, Integral]):
        ...
        assert tile_shape[0] * tile_shape[1] < max_tile, \
            f"tile dimensions {tile_shape} for image size {
                image_size} exceed maximum tile count (16 bit id), try increasing tile_size"
        ...
        with torch.no_grad():
            ...
            tile_ranges = torch.zeros(
                (*tile_shape, 2), dtype=torch.int32, device=gaussians.device)
            ...
            if total_overlap > 0:
                ...
                overlap_to_point = torch.empty(
                    (0, ), dtype=torch.int32, device=gaussians.device)
...
def map_to_tiles(gaussians: torch.Tensor, depth: torch.Tensor,
    ...
    assert gaussians.ndim == 2 and gaussians.shape[1] == Gaussian2D.vec.n, f"gaussians must be Nx{
        Gaussian2D.vec.n} got {gaussians.shape}"
    assert depth.ndim == 2 and depth.shape[1] == 1, f"depths must be Nx1, got {
        depth.shape}"
