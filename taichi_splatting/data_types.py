...
from taichi_splatting.torch_lib.transforms import (make_homog, split_rt,
                                                   transform44)
...
class RasterConfig:
    ...
    # compute visibility (pixels) for each gaussian
...
def check_packed3d(packed_gaussians: torch.Tensor):
    assert len(packed_gaussians.shape) == 2 and packed_gaussians.shape[1] == 11, f"Expected shape (N, 11), got {
        packed_gaussians.shape}"


def check_packed2d(packed_gaussians: torch.Tensor):
    assert len(packed_gaussians.shape) == 2 and packed_gaussians.shape[1] == 6, f"Expected shape (N, 6), got {
        packed_gaussians.shape}"
...
class Gaussians3D(TensorClass):
    ...
    def __post_init__(self):
        assert self.position.shape[1] == 3, f"Expected shape (N, 3), got {
            self.position.shape}"
        assert self.log_scaling.shape[1] == 3, f"Expected shape (N, 3), got {
            self.log_scaling.shape}"
        assert self.rotation.shape[1] == 4, f"Expected shape (N, 4), got {
            self.rotation.shape}"
        assert self.alpha_logit.shape[1] == 1, f"Expected shape (N, 1), got {
            self.alpha_logit.shape}"
    ...
    def concat_batch(gaussians: List['Gaussians3D']) -> 'Gaussians3D':
        ...
        dicts = {k: torch.cat([d[k] for d in dicts], dim=0)
