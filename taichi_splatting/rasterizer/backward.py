...
from taichi_splatting.taichi_lib.concurrent import (warp_add_vector_32,
                                                    warp_add_vector_64)
...
def backward_kernel(config: RasterConfig,
    ...
    assert block_area >= WARP_SIZE, \
        f"pixel_stride {config.pixel_stride} and tile_size {
            config.tile_size} must allow at least one warp sized ({WARP_SIZE}) tile"
    ...
    def _backward_kernel(
        # Input tensors
        # [N, 7] 2D gaussian parameters
        points: ti.types.ndarray(Gaussian2D.vec, ndim=1),
        # [N, F] gaussian features
        point_features: ti.types.ndarray(feature_vec, ndim=1),

        # Tile data structures
        # [T] start/end range of overlapping points
        tile_overlap_ranges: ti.types.ndarray(ti.math.ivec2, ndim=1),
        # [P] mapping from overlap index to point index
        overlap_to_point: ti.types.ndarray(ti.i32, ndim=1),

        # Image buffers
        # [H, W, F] output features
        image_feature: ti.types.ndarray(feature_vec, ndim=2),

        # Input image gradients
        # [H, W, F] gradient of output features
        grad_image_feature: ti.types.ndarray(feature_vec, ndim=2),

        # Output point gradients
        # [N, 7] gradient of gaussian parameters
        grad_points: ti.types.ndarray(Gaussian2D.vec, ndim=1),
        # [N, F] gradient of gaussian features
        grad_features: ti.types.ndarray(feature_vec, ndim=1),

        # Output point heuristics
        # [N] point densify heuristic
        point_heuristic: ti.types.ndarray(vec2, ndim=1),
    ):
        ...
        for tile_id, tile_idx in ti.ndrange(tiles_wide * tiles_high, block_area):
            ...
            tile_point = ti.simt.block.SharedArray(
                (block_area, ), dtype=Gaussian2D.vec)
            tile_point_id = ti.simt.block.SharedArray(
                (block_area, ), dtype=ti.i32)
            tile_feature = ti.simt.block.SharedArray(
                (block_area, ), dtype=feature_vec)
            ...
            for i, offset in ti.static(pixel_tile):
                ...
                if pixel.y < camera_height and pixel.x < camera_width:
                    ...
                    grad_pixel_feature[i,
                                       :] = grad_image_feature[pixel.y, pixel.x]
                    total_weight[i] = 0.0
            ...
            for point_group_id in range(num_point_groups):
                ...
                for in_group_idx in range(min(block_area, remaining_points)):
                    ...
                    mean, axis, sigma, point_alpha = Gaussian2D.unpack(
                        tile_point[in_group_idx])
                    has_grad = False

                    # Process all pixels in tile for current point
                    for i, offset in ti.static(pixel_tile):
                        pixel_saturated = total_weight[i] >= ti.static(
                            config.saturate_threshold)

                        pixelf = ti.cast(
                            pixel_base + ti.math.ivec2(offset), dtype) + 0.5
                        gaussian_alpha, dp_dmean, dp_daxis, dp_dsigma = pdf_with_grad(
                            pixelf, mean, axis, sigma)
                        alpha = point_alpha * gaussian_alpha

                        if alpha > ti.static(config.alpha_threshold) and not pixel_saturated:
                            has_grad = True

                            alpha = ti.min(alpha, ti.static(
                                config.clamp_max_alpha))
                            feature = tile_feature[in_group_idx]

                            # transmisivity (remaining weight)
                            ...
                            feature_diff = feature * T_i - \
                                remaining_features[i, :] / (1.0 - alpha)

                            alpha_grad_from_feature = feature_diff * \
                                grad_pixel_feature[i, :]
                    ...
                    if ti.simt.warp.any_nonzero(ti.u32(0xffffffff), ti.i32(has_grad)):
                        if ti.static(points_requires_grad):
                            warp_add_vector(
                                tile_grad_point[in_group_idx], grad_point)

                        if ti.static(features_requires_grad):
                            warp_add_vector(
                                tile_grad_feature[in_group_idx], grad_feature)

                        if ti.static(config.compute_point_heuristic):
                            warp_add_vector(
                                tile_point_heuristics[in_group_idx], gaussian_point_heuristics)
                ...
                if (load_index < end_offset):
                    ...
                    if ti.static(points_requires_grad):
                        ti.atomic_add(
                            grad_points[point_idx], tile_grad_point[tile_idx])

                    if ti.static(features_requires_grad):
                        ti.atomic_add(
                            grad_features[point_idx], tile_grad_feature[tile_idx])

                    if ti.static(config.compute_point_heuristic):
                        ti.atomic_add(
                            point_heuristic[point_idx], tile_point_heuristics[tile_idx])
