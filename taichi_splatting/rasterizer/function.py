...
def render_function(config: RasterConfig,
    ...
    forward = forward_kernel(
        config, feature_size=feature_size, dtype=torch_taichi[dtype])
    ...
    class _module_function(torch.autograd.Function):
        ...
        def forward(ctx, gaussians: torch.Tensor, features: torch.Tensor,
            ...
            image_feature = torch.empty(
                (*shape, features.shape[1]), dtype=dtype, device=features.device)
            image_alpha = torch.empty(
                shape, dtype=dtype, device=features.device)

            if config.compute_point_heuristic:
                point_heuristic = torch.zeros(
                    (gaussians.shape[0], 2), dtype=dtype, device=features.device)
            else:
                point_heuristic = torch.empty(
                    (0, 2), dtype=dtype, device=features.device)

            if config.compute_visibility:
                visibility = torch.zeros(
                    (gaussians.shape[0]), dtype=dtype, device=features.device)
            else:
                visibility = torch.empty(
                    (0), dtype=dtype, device=features.device)
...
def rasterize(gaussians2d: torch.Tensor, depth: torch.Tensor,
    ...
    assert gaussians2d.shape[0] == depth.shape[0] == features.shape[0], \
        f"Size mismatch: got {gaussians2d.shape}, {
            depth.shape}, {features.shape}"
