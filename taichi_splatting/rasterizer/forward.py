...
def forward_kernel(config: RasterConfig, feature_size: int, dtype=ti.f32):
    ...
    def _forward_kernel(
        ...
        # [N] visibility per point (if compute_visibility is True)
        visibility: ti.types.ndarray(dtype, ndim=1)
    ):
        ...
        for tile_id, tile_idx in ti.ndrange(tiles_wide * tiles_high, tile_area):
            pixel = tiling.tile_transform(
                tile_id, tile_idx, tile_size, (1, 1), tiles_wide)
            ...
            tile_point = ti.simt.block.SharedArray(
                (tile_area, ), dtype=Gaussian2D.vec)
            tile_feature = ti.simt.block.SharedArray(
                (tile_area, ), dtype=feature_vec)
            tile_point_id = ti.simt.block.SharedArray(
                (tile_area, ), dtype=ti.i32)
            ...
            for point_group_id in range(num_point_groups):
                ...
                for in_group_idx in range(min(tile_area, remaining_points)):
                    ...
                    if alpha > ti.static(config.alpha_threshold):
                        ...
                        if ti.static(config.use_alpha_blending):
                            ...
                            saturated = total_weight >= ti.static(
                                1.0 - config.saturate_threshold)
                    ...
                    if ti.static(config.compute_visibility):
                        if ti.simt.warp.any_nonzero(ti.u32(0xffffffff), ti.i32(alpha > ti.static(config.alpha_threshold))):
                            ...
                            warp_add_vector(
                                tile_visibility[in_group_idx], weight_vec)
                ...
                if ti.static(config.compute_visibility):
                    ...
                    if load_index < end_offset:
                        ...
                        ti.atomic_add(
                            visibility[point_idx], tile_visibility[tile_idx][0])
