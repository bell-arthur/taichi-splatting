...
def make_library(dtype=ti.f32):
    ...
    def project_with_jacobian(
        ...
        t = ti.math.clamp(uv, -image_size * clamp_margin,
                          (image_size - 1) * (1 + clamp_margin))
    ...
    def scaled_quat_to_mat(q: vec4, s: vec3) -> mat3:
        ...
        return mat3(
            s.x * (1 - 2 * y2 - 2 * z2), s.y * (2 * x * y -
                                                2 * w * z), s.z * (2 * x * z + 2 * w * y),
            s.x * (2 * x * y + 2 * w * z), s.y *
            (1 - 2 * x2 - 2 * z2), s.z * (2 * y * z - 2 * w * x),
            s.x * (2 * x * z - 2 * w * y), s.y *
            (2 * y * z + 2 * w * x), s.z * (1 - 2 * x2 - 2 * y2)
        )
