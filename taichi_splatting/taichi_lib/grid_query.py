...
import taichi as ti
from taichi.math import ivec2, mat2, vec2, vec3

from taichi_splatting.taichi_lib.f32 import Gaussian2D, ellipse_bounds
...
def make_grid_query(tile_size: int = 16, alpha_threshold: float = 1. / 255.):
    ...
    def obb_grid_query(v: Gaussian2D.vec, image_size: ivec2) -> OBBGridQuery:
        ...
        min_bound, max_bound = ellipse_bounds(
            mean, axis1 * scale.x, axis2 * scale.y)

        # transform from (relative) image to ellipse space
        inv_basis = ti.Matrix.rows([axis1 / scale.x, axis2 / scale.y])

        min_tile, max_tile = tile_ranges(
            min_bound, max_bound, image_size, tile_size)
        return OBBGridQuery(
            inv_basis=inv_basis,
