...
class ParameterClass():
    ...
    def __init__(self, tensors: TensorDict,
        ...
        assert tensors.batch_dims == 1 and tensors.batch_size[0] > 0, f"got {
            tensors.batch_size}"
        assert tensors.device is not None, f"tensors.device is None, got {
            tensors.device}"

        self.tensors: TensorDict = as_parameters(
            tensors, parameter_groups.keys())
        ...
        if optimizer_state is not None:
            ...
            for k in tensor_state.keys():
                assert k in self.tensors.keys(), f"state parameter {
                    k} not in {self.tensors.keys()}"
                self.optimizer.state[self.tensors[k]] = {
                    **tensor_state[k].to_dict(), **other_state[k]}
    ...
    def from_state_dict(state: dict, device: torch.device, optimizer=optim.Adam, **optim_kwargs) -> 'ParameterClass':
        ...
        return ParameterClass(
            TensorDict.from_dict(
                state['tensors'], batch_dims=1, device=device),
    ...
    def append_tensors(self, tensors: TensorDict, tensor_state: Optional[TensorDict] = None):
        if tensor_state is not None:
            assert tensors.shape == tensor_state.shape, f"{
                tensors.shape} != {tensor_state.shape}"

        assert set(tensors.keys()) == set(self.tensors.keys()), f"{
            tensors.keys()} != {self.tensors.keys()}"
        ...
        return ParameterClass(torch.cat([self.tensors, tensors]),
                              ...
                              optimizer_state=(
                                  torch.cat([self.tensor_state, tensor_state]), self.other_state),
                              optimizer=type(self.optimizer),
                              **self.optim_kwargs)
