...
from .util import get_scalar_state, get_total_weight, get_vector_state
...
class Group:
    ...
    def __repr__(self):
        indent = "    "
        state_str = f",\n{indent}".join(
            _format_state_item(k, v) for k, v in self.state.items())
...
def _format_tensor_stats(name: str, tensor: Optional[torch.Tensor]) -> str:
    ...
    use_scientific = tensor.abs().mean() < 1e-3
...
def make_group(group, state) -> Group:
    ...
    return Group(
        ...
        grad=params.grad.view(
            params.shape[0], -1) if params.grad is not None else None,
        state=state,
...
def weighted_step(group: Group,
    ...
    if group.type in ["vector", "local_vector"]:
        ...
        kernel = module.vector_kernel(betas=group.betas, eps=group.eps,
                                      dims=group.param.shape[1], bias_correction=group.bias_correction)
    elif group.type == "scalar":
        m, v = get_scalar_state(group.state, group.param)
        kernel = module.scalar_kernel(
            betas=group.betas, eps=group.eps, bias_correction=group.bias_correction)
    else:
    ...
    if group.type == "local_vector":
        ...
        group.grad[visible_indexes] = torch.einsum(
            'bij,bj->bi', inv_basis, group.grad[visible_indexes])

    lr_step = group.param.new_zeros(
        visible_indexes.shape[0], group.param.shape[1])
    kernel(lr_step,
...
class FractionalOpt(torch.optim.Optimizer):
    ...
    def __init__(self, kernels: types.ModuleType, param_groups: list[dict], lr=0.001,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps, mask_lr=None, point_lr=None,
                        type="scalar", bias_correction=bias_correction, clip=clip)
    ...
    def step(self,
        ...
        assert weight.shape == indexes.shape, f"shape mismatch {
            weight.shape} != {indexes.shape}"

        groups = [make_group(group, self.state) for group in self.param_groups]
        n = groups[0].param.shape[0]

        total_weight = get_total_weight(
            groups[0].state, n, device=weight.device)
...
class SparseAdam(FractionalOpt):
    ...
    def step(self, indexes: torch.Tensor, basis: Optional[torch.Tensor] = None):
        weight = torch.ones(
            indexes.shape[0], device=indexes.device, dtype=torch.float32)
